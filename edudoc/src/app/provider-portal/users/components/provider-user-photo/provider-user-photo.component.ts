...
@Component({
    selector: 'app-provider-user-photo',
    templateUrl: './provider-user-photo.component.html',
})
export class ProviderUserPhotoComponent implements OnInit {
    @Input() user: IUser;
    ...
    constructor(private notificationsService: NotificationsService, private userService: ProviderUserService) {}
    ...
    savePhoto(): void {
        if (this.croppedFile) {
            this.userService.savePhoto(this.user.Id, this.croppedFile).subscribe(() => {
